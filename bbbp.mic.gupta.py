...
bbbp_test_df= pd.read_csv('gupta_test.csv')
...
#print("Ghosh Accuracy: %.2f%%" % (ghosh_scores[1]*100))
...
gupta_scores = model.evaluate(Xtvecall, ytvecall, verbose=0)
print("Gupta Accuracy: %.2f%%" % (gupta_scores[1]*100))
auc_roc=roc_auc_score(ytvecall,model.predict(Xtvecall))
print('auc_roc=',auc_roc)

sens1, spec1 = confusion_matrix_scorer(model,Xtvecall,ytvecall)
print("sensitivity for Gupta test = ",sens1)
print("specificity for Gupta test = ", spec1)

print("pred = ",np.round(model.predict(Xtvecall)))
print("pred = ",(model.predict(Xtvecall)))

guptapreds = np.round(model.predict(Xtvecall))

accr = 0
for i in range(guptapreds.shape[0]):
    if ( guptapreds[i] == ytvecall[i] ):
        accr = accr + 1

pcent = (accr*100)/(guptapreds.shape[0]*1.0)
print("Calculated Gupta Accuracy with 0.1 threshold : %.2f%%" % (pcent))
